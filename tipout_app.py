...
with st.expander("👥 Staffing & Points", expanded=True):
    ...
    with col3:
        ...
        server_names_raw = st.text_input("Server Names (comma separated)", placeholder="Insert name")
        
        st.write("**Support Staff**")
        head_busser_names_raw = st.text_input("Head Busser Names (0.65 Pts each)", placeholder="Insert name")
    ...
    try:
        ...
        if num_bussers > 0:
            ...
            table_rows.append({
                ...
                "Notes": f"Total: ${round(busser_final_each * num_bussers, 2):,.2f} (0.6 pts ea)"
            })
        ...
        table_rows.append({
            ...
            "Notes": f"Split: ${bartender_each:,.2f} each"
        })
